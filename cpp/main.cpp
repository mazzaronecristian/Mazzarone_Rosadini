...
int main() {
    ...
    hero.push_back(std::make_shared<Player1>(factory.createHero(ElementType::hero)));
    LifeBar lifeBar = userFactory.createLifeBar(hero[0].get());

    srand(time(NULL));
    std::list<std::shared_ptr<Enemy>> enemies;
    for (int i = 0; i < 10; i++) {
        std::shared_ptr<Enemy> ghoul = std::make_shared<Enemy>(factory.createEnemy(ElementType::ghoul,
